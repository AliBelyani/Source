...
{
    public class ASPIdentityPasswordValidator<TUser> : ResourceOwnerPasswordValidator<TUser> where TUser : class
    {
        ...
        public ASPIdentityPasswordValidator(UserManager<TUser> userManager, SignInManager<TUser> signInManager, IEventService events, ILogger<ResourceOwnerPasswordValidator<TUser>> logger) : base(userManager, signInManager, events, logger)
        {
        ...
        public override async Task ValidateAsync(ResourceOwnerPasswordValidationContext context)
        {
            ...
            if (user != null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    context.Result = new GrantValidationResult(sub, IdentityModel.OidcConstants.AuthenticationMethods.Password);
                    return;
                }
                else if (result.IsLockedOut)
                {
                    ...
                    context.Result = new GrantValidationResult(TokenRequestErrors.InvalidGrant, "locked_out");
                }
